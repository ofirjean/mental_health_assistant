pipeline {
  ...
  environment {
    ...
    // remotes (HTTPS for PAT)
    GITHUB_REPO_HTTP  = 'https://github.com/ofirjean/mental_health_assistant.git'
    GITLAB_REPO_HTTP  = 'https://gitlab.com/sela-tracks/1116/students/ofir/final-project.git'
    ...
    stage('Bump Helm image.tag') {
      ...
      steps {
        sh '''
          ...
          echo "New image tag in $VALUES_FILE:"
      ...
      steps {
        withCredentials([usernamePassword(credentialsId: env.CRED_GITHUB_PAT, usernameVariable: 'GH_USER', passwordVariable: 'GH_PAT')]) {
          sh '''
            ...
            # force origin to use HTTPS+PAT (masked in logs)
    ...
    stage('Mirror to GitLab (PAT)') {
      ...
      steps {
        withCredentials([usernamePassword(credentialsId: env.CRED_GITLAB_PAT, usernameVariable: 'GL_USER', passwordVariable: 'GL_PAT')]) {
          sh '''
            # add/refresh gitlab remote with PAT (masked)
  ...
  post {
    always {
      script {
        echo "Done. Built & pushed ${env.DOCKER_REPO}:${env.TAG ?: 'N/A'}"
      }
    }
  }
}
